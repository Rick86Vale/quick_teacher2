...
# Importações dos seus modelos e formulários
from usuarios.views import eh_professor
from .models import (
    AreaConhecimento, Disciplina, Aula, Turma, 
    Instituicao, Aluno, Recurso
)
from .forms import (
    TurmaForm, InstituicaoForm, DisciplinaForm, 
    AreaConhecimentoForm, AulaForm, RecursoFormSet
)
...
def gerenciar_aulas(request, disciplina_id):
    ...
    # Proteção: Apenas o professor ou staff pode gerenciar
...
def criar_aula(request, disciplina_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            aula.save()
            messages.success(request, "Aula criada! Agora adicione os recursos extras.")
...
def gerenciar_recursos(request, aula_id):
    ...
    if request.method == 'POST':
        ...
        if formset.is_valid():
            formset.save()
            messages.success(request, "Recursos salvos com sucesso!")
            return redirect('visualizar_aula', aula_id=aula.pk)
    else:
        formset = RecursoFormSet(instance=aula)
        
    return render(request, 'academico/gerenciar_recursos.html', {
        'aula': aula,
        'formset': formset
    })
...
def alternar_publicacao(request, aula_id):
    ...
    if request.user == aula.disciplina.professor or request.user.is_staff:
        ...
        aula.save()
        messages.success(request, f"Aula '{aula.titulo}' foi {'publicada' if aula.publicado else 'ocultada'}.")
